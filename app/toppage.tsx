...
export default function PastExamSearchPage() {
  ...
  const handleSuggestionClick = (course: (typeof courses)[0]) => {
    window.open("https://example.com", "_blank")
  }

  // Handle category click
  const handleCategoryClick = (categoryName: string) => {
    const results = courses.filter((course) => course.category === categoryName)
    setSearchResults(results)
    setIsSearching(true)
    setSearchQuery("")
    setSuggestions([])
                ...
                {expandedCategories.has(category.id) && (
                  <div className="border-t border-border bg-muted/30">
                    {category.courses.map((course) => (
                      <button
                        key={course}
                        className="w-full p-3 pl-8 flex items-center gap-2 hover:bg-accent/10 transition-colors text-left"
